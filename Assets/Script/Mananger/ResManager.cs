...
public class ResManager : Singleton<ResManager>
{
    ...
    public UObject OnLoadAsset(string resName, string relativePath, Type assetType, ResType resType)
    {
        if (AppConst.DebugMode)
        {
            return ResLocalManager.Instance.LoadLocalUObject(relativePath, assetType, resType);
        }
        else
        {
            ...
            return AssetBundleManager.Instance.LoadAssetBundleUObject(resName, abName, assetName, assetType);
        }
    }

    //resName 资源卸载标识 Common为不卸载 其他通过标识卸载
    public void LoadAssetAsync(string resName, string relativePath, Type assetType, ResType resType, Action<UObject> sharpFunc = null, LuaFunction luaFunc = null)
    {

        if (AppConst.DebugMode)
        {
            string assetName = ResPath.GetEditorAssetName(relativePath, resType);
            ResLocalManager.Instance.LoadLocalUObjectAsync(relativePath, assetType, resType, sharpFunc, luaFunc);
        }
        else
        {
            string assetName = ResPath.GetAssetName(relativePath, resType);
            string abName = ResPath.GetAssetBunldeName(relativePath, resType);
            AddReloader(resName, abName);
            AssetBundleManager.Instance.LoadAssetBundleUObjectAsync(resName, abName, assetName, assetType, sharpFunc, luaFunc);
        }
    }
