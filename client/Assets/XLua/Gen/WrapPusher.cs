...
namespace XLua
{
    public partial class ObjectTranslator
    {
        ...
        class IniterAdderXLuaTestPedding
        {
			...
			static void Init(LuaEnv luaenv, ObjectTranslator translator)
			{
				...
				translator.RegisterPushAndGetAndUpdate<ResType>(translator.PushResType, translator.Get, translator.UpdateResType);
        ...
        int ResType_TypeID = -1;
		int ResType_EnumRef = -1;
        
        public void PushResType(RealStatePtr L, ResType val)
        {
            if (ResType_TypeID == -1)
            {
			    bool is_first;
                ResType_TypeID = getTypeId(L, typeof(ResType), out is_first);
				
				if (ResType_EnumRef == -1)
				{
				    Utils.LoadCSTable(L, typeof(ResType));
				    ResType_EnumRef = LuaAPI.luaL_ref(L, LuaIndexes.LUA_REGISTRYINDEX);
				}
				
            }
			
			if (LuaAPI.xlua_tryget_cachedud(L, (int)val, ResType_EnumRef) == 1)
            {
			    return;
			}
			
            IntPtr buff = LuaAPI.xlua_pushstruct(L, 4, ResType_TypeID);
            if (!CopyByValue.Pack(buff, 0, (int)val))
            {
                throw new Exception("pack fail fail for ResType ,value="+val);
            }
			
			LuaAPI.lua_getref(L, ResType_EnumRef);
        ...
        public void Get(RealStatePtr L, int index, out ResType val)
        {
		    LuaTypes type = LuaAPI.lua_type(L, index);
            if (type == LuaTypes.LUA_TUSERDATA )
            {
			    if (LuaAPI.xlua_gettypeid(L, index) != ResType_TypeID)
				{
				    throw new Exception("invalid userdata for ResType");
				}
                ...
                {
                    throw new Exception("unpack fail for ResType");
                }
				val = (ResType)e;
            ...
            {
                val = (ResType)objectCasters.GetCaster(typeof(ResType))(L, index, null);
            }
        }
		
        public void UpdateResType(RealStatePtr L, int index, ResType val)
        {
            ...
            if (LuaAPI.lua_type(L, index) == LuaTypes.LUA_TUSERDATA)
            {
                ...
                if (!CopyByValue.Pack(buff, 0,  (int)val))
                {
                    throw new Exception("pack fail for ResType ,value="+val);
                }
            }
	...
	public partial class StaticLuaCallbacks
    {
	    internal static bool __tryArrayGet(Type type, RealStatePtr L, ObjectTranslator translator, object obj, int index)
		{
			...
			else if (type == typeof(ResType[]))
			{
			    ResType[] array = obj as ResType[];
				translator.PushResType(L, array[index]);
				return true;
			}
