...
public class TaskFileList :ITask
{
    ...
    {
        var filesPath = ResPack.BuildPath + "/" + ResConst.CheckFile;
        List<string> lines = new List<string>();
        UTF8Encoding utf8 = new UTF8Encoding(false);
        if (File.Exists(filesPath)) File.Delete(filesPath);
        var files = Directory.GetFiles(ResPack.BuildPath, "*", SearchOption.AllDirectories);
        for (int j = 0; j < files.Length; j++)
        {
            ...
            if (string.IsNullOrEmpty(ext) || (ext != ".meta" && ext != ".manifest"))
            {
                ...
                string relativePath = file.Replace(ResPack.BuildPath, string.Empty).Substring(1);
